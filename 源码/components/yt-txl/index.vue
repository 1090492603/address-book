<template>
	<view class="yt-txl-container" >
		<view class="search" style="background-color: #F4F4F4;">
			<input type="text" @input="inputFunc" class="s-input" placeholder="搜索" />
		</view>
		<view class="scroll" :style="index ? '' :'right:25upx'">
			<scroll-view :scroll-into-view="to" scroll-y style="width: 100%;height: 100%;">
				<view :id="o.key" v-for="(o,i) in resource" :key="i">
					<view class="p" >{{o.key}}</view>
					<view @click="clickFunc(item)" v-for="(item,index) in o.data" class="info" :key="index" >
						<view style="background-color: #4CD964" class="icon">
							{{item[name].slice(0,1)}}
						</view>
						<view class="item" >
							<text class="name" style="background: #fff;">{{item.name}}</text>
							<text class="post" style="background: #fff;">{{item.number}}</text>
						</view>
					</view>
				</view>
			</scroll-view>
		</view>
		<view class="flag" v-if="index">
			<scroll-view scroll-y="true" :show-scrollbar="false" class="flag-scroll" style="width: 100%;height: 100%;">
				<view @click="toFunc(o.key)" class="flag-key" v-for="(o,i) in resource" :key="i">
					{{o.key}}
				</view>
			</scroll-view>
		</view>
		<button type="primary" class="btn1"  @click="goCreat">新建联系人</button>
	</view>
	
</template>

<script>
	'use strict';
	let list = new (require("./node.js"))();
	let pinyin = new(require('./pinyin'))({
		charCase: 0
	});

	export default {
		//传入参数props	
		props: {
			name: {
				type: String,
				default () {
					return "name";
				}
			},
			index: {
				type: Boolean,
				default: true
			},
			color: {
				type:String,
				default:"#f44336"
			}
		},
		data() {
			return {
				datas: [],
				resource: [],
				chars: [],
				to: "",
				change:{},
				cache:[]
			};
		},
		watch: {
			datas(r) {
				this._parseData(r)
			},
			change(c) {
				if (!(c instanceof Object)){
					console.log("the props datas type must be Object")
					return
				};
				this.changeOption(c);
			}
		},
		onLoad(option){
		 console.log(option.id)
		},
		methods: {
			toFunc(o) {
				this.to = o
			},
			clickFunc(item){
				uni.navigateTo({
					url:`./details?name=${item.name}&number=${item.number}&mail=${item.mail}&address=${item.address}&status=1`
				})
				
			},
			inputFunc(r){ //搜索功能 cache是全列表的数据记录 resource是当前列表显示
				if(!r.detail.value) {
					this.resource = this.cache
					return
				}
				let key  = r.detail.value;
				let temp = [];
				list.find(key,function(currNode){
					console.log("关键词  " +key+  " , 搜索到 "+currNode.name + ", 电话 ："+currNode.number);
				})
				this.cache.forEach(o => {
					o.data.forEach(item =>{
						if(item[this.name].indexOf(key) > -1 || item.number.indexOf(key) > -1){ // 匹配到
							// 确定当前这个元素的key是谁
							let key = o.key
							// 找到temp中的key
							let index = 0 //下标
							let find = false // 数据是否存在
							for(let d in temp){
								if(temp[d].key === o.key){
									index = d
									find = true
								}
							}
							if(find){ // 如果key已经存在，直接插入数据
								temp[index].data.push(item)
							} else { // 不存在初始化一个并存入
								temp.push({
									key:o.key,
									data:[item]
								})
							}
						}
					})
				})
				this.resource = temp
			},
			_type(val){
				return Object.prototype.toString.call(val).slice(8,-1).toLowerCase()
			},
			_parseData(r) {
				// 生成a-z的数组
				let data = [];
				this.chars = []
				for (let i = 65; i <= 90; i++) {
					let key = String.fromCharCode(i)
					data.push({"key":key,data:[]})
					this.chars.push(key)
				}
				if(this._type(r) === "array") {
					
					// 填充数据
					r.forEach(o => {
						// 找到char的位置
						data.forEach( (item,index) => {
							let a = this._parseChar(o[this.name || 'name'])
							if(item.key === a){
								data[index].data.push(o)
							}
						})
					})
				}
				// 组合最后数据并踢出没有匹配到a-z中的任意数据
				let finalData = []
				for (let i in data) {
					if(data[i].data.length > 0){
						finalData.push(data[i])
					}
				}
				this.resource =finalData;
				this.cache = finalData;
				finalData = null
			},
			_parseChar(name) {
				if (Object.prototype.toString.call(name).slice(8, -1) !== 'String') {
					console.error("name is not string")
					return
				}
				let chars = pinyin.getFullChars(name);
				return chars[0].toUpperCase()
			},
			goCreat(){
				//status 0是新建 1是修改 2是删除
				uni.navigateTo({
					url:`./details?status=0`
				})
			},
			changeOption(data){
				if(data.status === 0){
					this.insertNode(data);
				}	
				if(data.status === 1){
					this.changeNode(data);
				}
				if(data.status === 2){
					this.removeNode(data);
				}
			},
			insertNode(data){
				if(list.findNumber(data.number) != 1){
					this.$emit("ev","电话号码已存在");
					return ;
				}
				let newNode = list.insert(data.name,data.number,data.mail,data.address);
				this.datas.push(newNode);
				this.setStorageData();
				this.$emit("ev","新建成功");
			},
			changeNode(data){

				let index = -1;
				let node = list.findNumber(data.number);
				
				if(data.oldNumber){
					//要修改手机号才会显示oldNumber;
					index = this.datas.findIndex(e=> e.number=== data.oldNumber);
					let oldNode = list.findNumber(data.oldNumber);
					if(node != 1){
						this.$emit("ev","电话号码已存在");
						return ;
					}else{
						console.log(data.number,data.oldNumber);
						oldNode.name = data.name;
						oldNode.number = data.number;
						oldNode.mail = data.mail;
						oldNode.address = data.address;
						this.datas.splice(index,1);
						this.datas.push(oldNode);
						this.setStorageData();
						this.$emit("ev","修改成功");
					}
				}else{
						index = this.datas.findIndex(e=> e.number=== data.number);
						node.name = data.name;
						node.mail = data.mail;
						node.address = data.address;
						this.datas.splice(index,1);
						this.datas.push(node);
						this.setStorageData();
						this.$emit("ev","修改成功");

				}
			},
			removeNode(data){
				list.remove(data.number);
				this.datas.splice(this.datas.findIndex(e=> e.number=== data.number) , 1 );
				this.setStorageData();
				this.$emit("ev","删除成功");
			},
			setStorageData(){
				uni.setStorage({
				    key: 'datas',
				    data: JSON.stringify(this.datas),
				    success: function () {
				        console.log('缓存成功');
				    }
				});
			},
			getStorageData(){
				var _this = this;
				uni.getStorage({
				    key: 'datas',
				    success: function (res) {
						res = JSON.parse(res.data);
						res.forEach(o=>{
							list.insert(o.name,o.number,o.mail,o.address);
						})
						list.dispaly();
				        _this.datas = res;
				    }
				});
			}
		},
		
		created() {
			var _this = this;
			console.log(this.datas);
			uni.$on('de',function(data){
								_this.change = data;
			})
			
		},
		mounted(){
			console.log("重新挂载对象");
			this.getStorageData();
		}
	};
</script>

<style>
	page {
		background: #f4f4f4;
	}
	.search {
		width: 100%;
		height: 120upx;
		display: flex;
		justify-content: center;
		align-items: center;
	}

	.s-input {
		width: 700upx;
		height: 80upx;
		background: url('data:image/png;base64,') 20upx center no-repeat #fff;
		background-size: 40upx;
		text-indent: 80upx;
	}

	.scroll {
		position: absolute;
		left: 25upx;
		top: 120upx;
		right: 100upx;
		bottom: 25upx;
		padding-bottom: 150upx;
	}

	.p {
		position: sticky;
		top: 0;
		left: 0;
		background: #f4f4f4;
		font-size: 28upx;
		margin-bottom: 10upx;
		text-indent: 40upx;
		z-index: 99998;
		font-weight: bold;
	}

	.info {
		display: flex;
		justify-content: flex-start;
		align-items: center;
		margin-bottom: 10upx;
		padding: 20upx 25upx;
		background: #fff;
	}

	.icon {
		width: 100upx;
		height: 100upx;
		border-radius: 50%;
		margin-right: 20upx;
		display: flex;
		justify-content: center;
		align-items: center;
		font-size: 30upx;
		color: #fff;
	}

	.item {
		height: 100upx;
		display: flex;
		flex-direction: column;
		justify-content: space-around;
	}

	.flag {
		width: 50upx;
		position: absolute;
		top: 120upx;
		right: 25upx;
		bottom: 25upx;
	}

	.flag-scroll {
		padding-top: 10upx;
	}

	.flag-key {
		padding: 0;
		margin: 0 auto 10upx auto;
		width: 30upx;
		height: 30upx;
		border-radius: 50%;
		color: #000;
		font-weight: bold;
		display: flex;
		justify-content: center;
		align-items: center;
		font-size: 20upx;
	}
	
	.btn1{
		position: fixed;
		right: 5%;
		left: 5%;
		bottom: 50upx;
	}
</style>
